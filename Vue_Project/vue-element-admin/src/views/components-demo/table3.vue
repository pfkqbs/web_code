  ...
  data() {
    return {
      ...
      filterData: {
        timeSelect: true,
        elinput: [
          { name: '脸影号', width: 230, key: 'userId' },
          { name: '动态ID', width: 230, key: 'dynamicId'  },
          { name: '分享ID', width: 230, key: 'shareId' }
        ]
      },
      // 表格配置
      dataSource: {
        ...
        data: [], // 表格数据
        cols: [
          { label: '头像', prop: 'headImg',  isImagePopover: true, width: 70 },
          { label: '动态id', prop: 'dynamicId' , width: 100 },
          { label: '分享id', prop: 'id', width: 100 },
          { label: '分享人脸影号', prop: 'userId' },
          { label: '分享人昵称', prop: 'nickName' },
          { label: '分享的时间', prop: 'shareTime',
            isCodeTableFormatter: function(val) { return timeFormat(val.shareTime) }
          },
          { label: '过期时间', prop: 'expireTime',
            isCodeTableFormatter: function(val) {  return timeFormat(val.expireTime) }
          },
          {  label: '分享渠道', prop: 'shareChannel',
